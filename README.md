...
const Logo = () => {
  return <img src={logo} alt="jobtrak" className="logo" />;
};
    ...
    <Wrapper>
      <form className="form">
        ...
        <FormRow
          type="email"
          name="email"
          defaultValue="sachinbharwad123@gmail.com"
        />
...
- setup a basic server which listening on PORT=6200
...
```js
fetch("http://localhost:6200/api/v1/test")
  .then((res) => res.json())
...
target: 'http://localhost:6200/api': This specifies the target URL where the requests will be redirected. In this case, any request that matches the /api path will be forwarded to http://localhost:6200/api.
...
To summarize, these lines of code configure a proxy rule for requests starting with /api on the development server. The requests will be redirected to http://localhost:6200/api, with the /api prefix removed from the path.
...
const Login = () => {
  ...
  const loginDemoUser = async () => {
    ...
    try {
      await customFetch.post('/auth/login', data);
      toast.success('welcome bot to test drive');
      navigate('/dashboard');
    } catch (error) {
      toast.error(error?.response?.data?.msg);
    }
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 50 }}>
        ...
        <YAxis allowDecimals={false} />
        <Tooltip />
        <Bar dataKey="count" fill="#2cb1bc" barSize={55} />
      </BarChart>
    </ResponsiveContainer>
  );
};
...
import Job from "./Job";
import Wrapper from "../assets/wrappers/JobsContainer";
import PageBtnContainer from "./PageButton";
import { useAllJobsContext } from "../pages/AllJobs";
